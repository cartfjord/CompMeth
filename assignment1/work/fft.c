...
QAM_input(struct complex *data,double amp,int32_t N,int32_t Nu,int8_t M) {
...
void fft_distortion_test(int32_t N,                              // dimension of FFT under test 
			 ...
			 int8_t *scale,                        // pointr to scaling schedule
			 double *maxSNR,                     // pointer best signal-to-noise ratio
			 int8_t *maxscale,                     // pointer to best scaling schedule
...
void main(int32_t argc, int8_t *argv[])
{
  ...
  int32_t    i;
  int8_t scale[7],maxscale[7],MAXSHIFT;
